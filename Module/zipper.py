...
class SeedZip:
    ...
    def create_zip(self) -> SeedZipResult:
        ...
        with ZipFile(zip_data, "w", ZIP_DEFLATED) as out_zip:
            ...
            mod.add_base_messages(
                ...
                player_number = settings.coop_player_number if settings.coop_hints_enabled else 0
            )
            self.prepare_attack_data_randomization(mod)
    ...
    def prepare_attack_data_randomization(self, mod: SeedModBuilder):
        ...
        damage_preset = ui_settings.get(keys.ATTACK_DATA_DAMAGE_PRESET)
        element = ui_settings.get(keys.ATTACK_DATA_ELEMENT)
...
class CosmeticsOnlyZip:
    ...
    def create_zip(self) -> io.BytesIO:
        ...
        with ZipFile(data, "w", ZIP_DEFLATED) as out_zip:
            mod = ModYml(title="Randomized Cosmetics", description=ModYml.default_generated_mod_description())
...
class BossEnemyOnlyZip:
    ...
    def create_zip(self) -> io.BytesIO:
        ...
        data = io.BytesIO()
        with ZipFile(data, "w", ZIP_DEFLATED) as out_zip:
            mod = ModYml(title="Randomized Bosses/Enemies", description=ModYml.default_generated_mod_description())
